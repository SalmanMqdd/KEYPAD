...
Password password = Password( "342019" ); //angak kode PIN 
...
Keypad miqdad = Keypad( makeKeymap(keys), PinBaris,   PinKolom, JmlBaris, JmlKolom ); 
void setup() { 
  ...
  delay(500);
  lcd.clear();
  miqdad.addEventListener(keypadEvent);
}
void loop() {
  pinMode(led, OUTPUT);
   if (a==0) { //state awal
  ...
  miqdad.getKey(); //membaca keypad
}
void keypadEvent(KeypadEvent eKey){ 
  switch (miqdad.getState()){
